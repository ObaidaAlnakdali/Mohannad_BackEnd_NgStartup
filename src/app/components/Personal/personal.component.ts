...
export class PersonalComponent implements OnInit, OnDestroy {
  ...
  fetchData() {
    ...
    this.Get_Personal_By_Where_Subscription = this.proxy
      .Get_Personal_By_Where(this.searchModel)
      .subscribe((result) => {
        if (result != null) {
    ...
    this.proxy.Edit_Personal(current).subscribe((result) => {
      if (result != null) {
        ...
        if (current.PERSONAL_ID === -1) {
          ...
          newEntry.MyURL =
            this.CmSvc.APIUrl +
            "/Upload_Image?REL_ENTITY=[TBL_PERSONAL]&REL_FIELD=PERSONAL_IMAGE&REL_KEY=" +
            newEntry.PERSONAL_ID;
  ...
  Delete(entry) {
    ...
    dialogRef.afterClosed().subscribe((response) => {
      if (response) {
        const _params_Delete_Personal = new Params_Delete_Personal();
        _params_Delete_Personal.PERSONAL_ID = entry.PERSONAL_ID;
        this.proxy
          .Delete_Personal(_params_Delete_Personal)
          .subscribe((data) => {
            if (data === "") {
