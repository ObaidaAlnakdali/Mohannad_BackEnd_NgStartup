...
Proxy,
Publisher,
Params_Get_Publisher_By_Where,
Params_Delete_Publisher,
...
Get_Publisher_By_Where_Subscription = new Subscription();
searchModel: Params_Get_Publisher_By_Where = new Params_Get_Publisher_By_Where();
data: Publisher[] = [];
...
ngOnDestroy(): void {
this.Get_Publisher_By_Where_Subscription.unsubscribe();
...
this.Get_Publisher_By_Where_Subscription = this.proxy.Get_Publisher_By_Where(this.searchModel).subscribe(result => {
 if (result != null) {
result.forEach((element: any) => {
element.MyURL = this.CmSvc.APIUrl + '/Upload_Image?REL_ENTITY=[TBL_PUBLISHER]&REL_FIELD=PUBLISHER_IMAGE&REL_KEY=' + element.PUBLISHER_ID;
...
if (this.data.filter(e => e.PUBLISHER_ID === -1).length > 0) {
return;
}
}
const record = new Publisher();
record.PUBLISHER_ID = -1;
record.ACTIVE = false;
this.data.unshift(record);
}
Edit(current) {
this.proxy.Edit_Publisher(current).subscribe((result) => {
if (result != null) {
this.CmSvc.ShowMessage('Done');
if (current.PUBLISHER_ID === -1) {
this.data.splice(this.data.indexOf(current), 1);
const newEntry: any = result;
newEntry.MyUploadedImages = [];
newEntry.MyURL = this.CmSvc.APIUrl + '/Upload_Image?REL_ENTITY=[TBL_PUBLISHER]&REL_FIELD=PUBLISHER_IMAGE&REL_KEY=' + newEntry.PUBLISHER_ID;
...
if (response) {
const _params_Delete_Publisher = new Params_Delete_Publisher();
_params_Delete_Publisher.PUBLISHER_ID = entry.PUBLISHER_ID;
this.proxy.Delete_Publisher(_params_Delete_Publisher).subscribe(data => {
if (data === '') {
